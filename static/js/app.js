 ...
 * ================================
 * Card rendering, task CRUD, search, filter, sort, bulk actions,
 * stats, draft recovery, and edit state recovery.
 */
(() => {
  "use strict";
  ...
  // Avatar pool (inline SVG data URIs)
  const AVATARS = [
  ...
  function renderCards() {
    ...
    // Update stats from all cards (not filtered)
    updateStats();
  ...
  // Enter key to submit add-task and inline edits
  ...
  async function handleDeleteCard(cardId) {
    // SweetAlert2 confirmation — non-blocking, async/await based
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "This card and all its tasks will be permanently deleted.",
      icon: "warning",
    ...
    try {
      ...
      renderCards();
      Swal.fire({ title: "Deleted!", text: "Card has been deleted.", icon: "success", timer: 1500, showConfirmButton: false });
    } catch (err) {
  ...
  async function handleDeleteAll() {
    // SweetAlert2 confirmation — extra cautious for bulk delete
    const result = await Swal.fire({
      title: "Delete ALL cards?",
      text: "This action cannot be undone. Every card and task will be permanently removed.",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#ef4444",
      cancelButtonColor: "#6b7280",
    ...
    try {
      ...
      cards = [];
      renderCards();
      Swal.fire({ title: "All Clear!", text: "All cards have been deleted.", icon: "success", timer: 1500, showConfirmButton: false });
    } catch (err) {
      Swal.fire({ title: "Error", text: String(err), icon: "error" });
    }
  }
